...
function populateForm(individualElements, summary, data) {
  ...
  individualElements.forEach((element) => {
    ...
    newInputRow.innerHTML = `
            <form>
                <label for="rowNumber" class="rowNumber">${
                  element.rowNumber
                }</label>
                <label for="name" class="nameLabel">Ime elementa:</label>
                <input type="text" class="name valueInput" value="${
                  element.name
                }">
                <label for="length" class="lengthLabel">Duzina (mm):</label>
                <input type="number" class="length valueInput" value="${
                  element.length
                }">
                <label for="width" class="widthLabel">Sirina (mm):</label>
                <input type="number" class="width valueInput" value="${
                  element.width
                }">
                <label for="result" class="resultLabel">Povrsina (m2):</label>
                <input type="number" class="result valueInput" value="${
                  element.area
                }"
                <label for="lengthSide" class="lengthSideLabel">Kant po duzini:</label>
                <input type="checkbox" class="lengthSideCheckbox valueInput" ${
                  element.lengthSide1 ? "checked" : ""
                }>
                <input type="checkbox" class="lengthSideCheckbox valueInput" ${
                  element.lengthSide2 ? "checked" : ""
                }>
                <label for="widthSide" class="widthSideLabel">Kant po sirini:</label>
                <input type="checkbox" class="widthSideCheckbox valueInput" ${
                  element.widthSide1 ? "checked" : ""
                }>
                <input type="checkbox" class="widthSideCheckbox valueInput" ${
                  element.widthSide2 ? "checked" : ""
                }>
                <label for="sideProcessingLength" class="sideProcessingLabel">Duzina kantovanja (m):</label>
                <input type="number" class="sideProcessingresult valueInput" value="${
                  element.sideProcessingLength
                }">
            </form>
        `;
  ...
  let sideProcessingLengthPlusWaste = document.querySelector(
    ".sideProcessingField"
  );
  ...
  let saveButtonWholeBoardOption = document.querySelector(
    ".saveWholeBoardOption"
  );
  let writeButtonWholeBoardOption = document.querySelector(
    ".writeWholeBoardOption"
  );
  ...
  if (data.isWholeBoardMandatory === "Yes") {
    ...
    sideProcessingLengthPlusWaste.value =
      summary.totalSideProcessingLengthWithWaste;
